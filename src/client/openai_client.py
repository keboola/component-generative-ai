...
class OpenAIClient(AsyncOpenAI, CommonClient):
    ...
    async def get_inference_function(self, model_name: str) -> Callable:
        ...
        try:
            await self.get_chat_completion_result(
                model_name,
            ...
            logging.warning(
                f"Cannot use chat_completion endpoint for model {model_name}, the component will try to use"
                f"completion_result endpoint."
            )

        try:
            await self.get_completion_result(
                model_name,
        ...
        if not content:
            if response.choices[0].finish_reason == "content_filter":
                raise AIClientException(
                    f"Cannot process prompt: {prompt}\nReason: content_filter\n"
